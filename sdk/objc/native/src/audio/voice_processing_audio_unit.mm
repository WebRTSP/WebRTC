...
std::atomic_bool VoiceProcessingAudioUnit::mic_owning_(false);
...
bool VoiceProcessingAudioUnit::CanRecord() const {
  ...
  return
    ...
    !mic_owning_.load(std::memory_order_relaxed);
}
...
bool VoiceProcessingAudioUnit::Initialize(Float64 sample_rate, bool enable_playout, bool enable_recording)
{
  ...
  bool expected_mic_owning = false;
  if (enable_recording && !mic_owning_.compare_exchange_strong(expected_mic_owning, true, std::memory_order_relaxed)) {
