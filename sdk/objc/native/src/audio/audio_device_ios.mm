...
int32_t AudioDeviceIOS::StartRecording() {
  ...
  if(!audio_unit_->CanRecord()) {
      RTCLogInfo(@"VoiceProcessingAudioUnit is not ready to start recording");
      return -1;
  }
  ...
  // StartRecording() can lead to audio unit recreate and io thread may be different after
  io_thread_checker_.Detach();
...
void AudioDeviceIOS::HandleSampleRateChange() {
  ...
  if (!audio_unit_->Initialize(playout_parameters_.sample_rate(), playing_.load(), recording_.load())) {
...
void AudioDeviceIOS::UpdateAudioUnit(bool can_play_or_record) {
  ...
  switch (audio_unit_->GetState()) {
    case VoiceProcessingAudioUnit::kUninitialized:
      ...
      should_initialize_audio_unit = can_play_or_record && (playing_.load() || recording_.load());
